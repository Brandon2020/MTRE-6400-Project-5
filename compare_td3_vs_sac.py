...
Creates a professional figure comparing TD3 and SAC+HER paths (for report Fig. 1)
"""
...
def get_trajectory(model, seed=42):
    """Run one episode and record the ant's (x,y) positions"""
...
for ax, model, title, color in zip([ax1, ax2], [td3_model, sac_model], ["TD3", "SAC+HER"], ['red', 'green']):
    ...
    # Goal
    ax.plot(5.5, 5.5, marker='*', markersize=25, color='white', 
...
plt.suptitle("TD3 vs SAC + HER Trajectory Comparison on AntMaze_Medium-v5", fontsize=16)
plt.tight_layout()
plt.savefig("td3_vs_sac_her_trajectories.png", dpi=300, bbox_inches='tight')
plt.show()

print("✅ Side-by-side trajectory comparison saved!")
